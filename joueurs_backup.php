...
$sql = "
    SELECT u.*, j.*, 
           (SELECT AVG(rating) FROM evaluations WHERE evaluated_id = u.id) as avg_rating,
           (SELECT COUNT(*) FROM evaluations WHERE evaluated_id = u.id) as rating_count
    FROM users u 
...
<body class="bg-light">
                        ...
                        <div class="card h-100 shadow-sm joueur-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <div class="rating">
                                        <?php
                                        $rating = round($joueur['avg_rating'] ?? 0);
                                        for ($i = 1; $i <= 5; $i++): ?>
                                            <i class="fas fa-star <?php echo $i <= $rating ? 'text-warning' : 'text-muted'; ?>"></i>
                                        <?php endfor; ?>
                                        <small class="text-muted ms-1">(<?php echo $joueur['rating_count'] ?? 0; ?>)</small>
                                    </div>
                                </div>
                                ...
                                <?php if (!empty($joueur['video_url'])): ?>
                                    <div class="mb-3">
                                        <video controls class="w-100" style="max-height: 150px;">
                                            <source src="<?php echo htmlspecialchars($joueur['video_url']); ?>" type="video/mp4">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <?php if (isset($_SESSION['user_id']) && $_SESSION['user_type'] !== 'joueur'): ?>
                                        <button class="btn btn-primary btn-sm" 
                                                onclick="contactPlayer(<?php echo $joueur['user_id']; ?>)">
    ...
    <script>
        function viewProfile(userId) {
            window.location.href = `joueur-profile.php?id=${userId}`;
        }

        function contactPlayer(userId) {
            // Ouvrir une modale de contact ou rediriger vers la page de contact
            window.location.href = `contact.php?player_id=${userId}`;
        }
